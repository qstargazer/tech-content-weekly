...
def _display_date(published: datetime, now: datetime) -> str:
    tz = now.tzinfo
    if tz is not None and published.tzinfo is not None:
        published = published.astimezone(tz)
    return f"{published:%m-%d}"


def _table_row(item: ContentItem, now: datetime) -> str:
    ...
    return (
        ...
        f"<td>{_display_date(item.published, now)}</td>"
        f"<td>{metrics}</td>"
        "</tr>"
    )


def _card(item: ContentItem, now: datetime, rank: int | None = None) -> str:
    ...
    return (
        ...
        f'{html.escape(PLATFORM_NAMES[item.platform])} · {_display_date(item.published, now)}</div>'
...
def _recommendation_card(rec: Recommendation, now: datetime, rank: int | None = None) -> str:
    badge = f'<span class="rank">{rank}</span>' if rank else ""
    item = rec.item
    label = RECOMMENDATION_LABELS.get(rec.category, rec.category)
    return (
        f'<article class="item">{badge}<div class="item-main"><div class="item-meta">'
        f'{html.escape(PLATFORM_NAMES[item.platform])} · {_display_date(item.published, now)} · '
        f'<span class="rec-tag">{html.escape(label)}</span></div>'
        f'<h3><a href="{html.escape(item.url, quote=True)}">{html.escape(item.title)}</a></h3>'
        f'<p class="rec-reason">{html.escape(rec.reason)}</p>'
        f'<div class="metrics"><span>时长 {_duration(item.duration_seconds)}</span>'
        f'<span>播放 {_count(item.view_count)}</span></div>'
        "</div></article>"
    )
...
def _recommendations_html(
    ...
    now: datetime,
    rec_provider: str | None,
    rec_model: str | None,
) -> str:
    if not recommendations and not top_pick:
        return ""
    top_html = ""
    if top_pick:
        top_html = (
            '<div class="top-pick"><h3>本周最值得投入</h3>'
            f"{_recommendation_card(top_pick, now, 1)}</div>"
        )
    groups = []
    for category in (CATEGORY_COMMUTE, CATEGORY_DEEP):
        rows = [rec for rec in recommendations if rec.category == category]
        cards = "".join(_recommendation_card(rec, now, index) for index, rec in enumerate(rows, 1))
        empty = '<p class="empty">本周没有该场景的内容。</p>'
        groups.append(
            f'<details open class="detail"><summary>{html.escape(RECOMMENDATION_LABELS[category])} '
            f"<span>{len(rows)}</span></summary>{cards or empty}</details>"
        )
    attribution = (
        f'<div class="attribution">场景分类由 {html.escape(rec_provider)} 模型 <code>{html.escape(rec_model or "")}</code> 生成；'
        "仅供参考。</div>"
        if rec_provider else ""
    )
    return (
        '<section class="recommend"><h2>本周推荐</h2>'
        f"{top_html}{''.join(groups)}{attribution}</section>"
    )


def _douban_rankings_html(items: list[ContentItem], now: datetime) -> str:
    books = [item for item in items if item.platform == "douban"]
    if not books:
        return ""
    ...
    blocks = []
    for creator_name, rows in groups.items():
        cards = "".join(_card(item, now, index) for index, item in enumerate(rows, 1))
        blocks.append(
            f'<details open class="detail douban-rank"><summary>{html.escape(creator_name)} '
            f'<span>{len(rows)}</span></summary>{cards}</details>'
        )
    return (
        '<section class="douban"><h2>豆瓣读书榜单 <span>完整榜单</span></h2>'
        '<p class="section-note">榜单来自豆瓣新书速递，按榜单顺序展示完整榜单；评分来自豆瓣公开数据。</p>'
...
def render_html(
    ...
    for platform in ("bilibili", "youtube", "podcast", "douban"):
        ...
        cards = "".join(_card(item, now) for item in rows) or '<p class="empty">本周没有新内容。</p>'
    ...
    for creator in creators:
        if not creator.enabled or creator.platform == "douban":
            continue
        ...
        if creator.platform == "podcast":
            ...
            note = "按报告生成时的公开播放量排序。"
        cards = "".join(_card(item, now, index) for index, item in enumerate(rows, 1)) or '<p class="empty">最近 30 天没有可用内容。</p>'
    ...
    schedule_line = f'<p class="schedule">{html.escape(schedule_note)}</p>' if schedule_note else ""
    platform_nav = "".join(
    ...
    weekly_table = (
        ...
        + "".join(_table_row(item, now) for item in weekly)
        + "</tbody></table></div>"
    ...
    recommend_html = _recommendations_html(recommendations or [], top_pick, now, rec_provider, rec_model)
    douban_html = _douban_rankings_html(items, now)
    warning_html = ""
    if warnings:
...
def render_markdown(
    ...
    lines = [f"# {title}", "", f"> 生成日期：{now:%Y-%m-%d}", ""]
    if schedule_note:
        lines.extend([f"> 发送频率：{schedule_note}", ""])
    lines.append("## 本周更新")
    for item in weekly:
        if item.platform == "douban":
            rating = _book_rating(item)
    ...
    if recommendations or top_pick:
        lines.extend(["", "## 本周推荐"])
        if top_pick:
            lines.append(f"### 本周最值得投入\n- **{top_pick.item.creator_name}**｜[{top_pick.item.title}]({top_pick.item.url})｜{top_pick.reason}")
        for category in (CATEGORY_COMMUTE, CATEGORY_DEEP):
            rows = [rec for rec in recommendations or [] if rec.category == category]
            if not rows:
                continue
            lines.append(f"### {RECOMMENDATION_LABELS[category]}")
            lines.extend(f"- **{rec.item.creator_name}**｜[{rec.item.title}]({rec.item.url})｜{rec.reason}" for rec in rows)
        if rec_provider:
            lines.extend(["", f"> 场景分类由 {rec_provider} 模型 `{rec_model or ''}` 生成，仅供参考。"])
    douban_books = [item for item in items if item.platform == "douban"]
    if douban_books:
        lines.extend(["", "## 豆瓣读书榜单"])
        lines.extend(["", "> 榜单来自豆瓣新书速递，按榜单顺序展示完整榜单；评分来自豆瓣公开数据。"])
        for book in douban_books:
            rating = _book_rating(book)
            lines.append(f"- **{book.creator_name}**｜[{book.title}]({book.url})｜评分 {rating or '未公开'}")
    if insight:
