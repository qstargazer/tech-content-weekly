...
def _douban_rankings_html(items: list[ContentItem], now: datetime) -> str:
    ...
    return (
        ...
        '<p class="section-note">榜单来自豆瓣热门图书与新书速递，按榜单顺序展示完整榜单；评分来自豆瓣公开数据。</p>'
...
</style></head><body><div class="shell"><article class="report"><header class="hero"><p class="eyebrow">BILIBILI · YOUTUBE · PODCAST · DOUBAN</p><h1>{html.escape(title)}</h1><p class="subtitle">{subtitle}</p>{schedule_line}</header><main class="content"><section class="overview"><h2>本周概览</h2><p>先用表格扫完所有更新；需要细看时，使用下方目录跳转到平台或创作者详情，或直接打开原始页面。</p><nav class="nav">{platform_nav}</nav><nav class="nav creator">{creator_nav}</nav></section>{insight_html}{recommend_html}<section><h2>本周更新 <span>{len(weekly)}</span></h2>{weekly_table}</section><section><h2>按平台展开</h2>{''.join(platform_sections)}</section>{douban_html}<section><h2>创作者近 30 天</h2>{''.join(creator_sections)}</section>{warning_html}<div class="notice">数据说明：播放量与评论数是报告生成时的公开快照；播客公开 RSS 通常没有统一播放和评论指标。单个来源失败时使用缓存并明确标注，不影响其他创作者。</div>{attribution}</main><footer class="footer">多平台科技内容长期追踪 · 重要内容请回到原始页面核验</footer></article></div></body></html>'''
...
def render_markdown(
    ...
    if douban_books:
        lines.extend(["", "## 豆瓣读书榜单"])
        lines.extend(["", "> 榜单来自豆瓣热门图书与新书速递，按榜单顺序展示完整榜单；评分来自豆瓣公开数据。"])
